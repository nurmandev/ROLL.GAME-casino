...
@Component({
  selector: 'app-limbo-history',
  templateUrl: './limbo-history.component.html',
  styleUrls: ['./limbo-history.component.scss']
})
export class LimboHistoryComponent implements OnInit {
  ...
  constructor(private conn: ConnectionService, private alert: ToastrService, private route: Router, private actRoute: ActivatedRoute) {
      if (actRoute.snapshot.url[1].path =='limbo') {
  ...
  details(){
    var obj={'_id':this.id,game:'limbo'}
    this.conn.postRequest('history/limboDetails',obj,this.token).subscribe((resData:any) => {
