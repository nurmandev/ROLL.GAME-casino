...
isLoading = false;table:any=[];p:any = 0;limit:any = 10;totalCount:any;page:any = 0;
  ...
  list(){
    ...
    this.dataService.loadData('history/gameList',params).subscribe((resData:any)=>{
  ...
  update(id:any,action:any){
    var act=(action == 1) ? "activate" : "de-activate" ;
    var obj={id:id,status:act}
    this.dataService.postRequest('history/editGamelist',obj,this.token).subscribe((resData:any)=>{
      if(resData.status==true){
