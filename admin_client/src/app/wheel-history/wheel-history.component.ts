...
@Component({
  ...
  templateUrl: './wheel-history.component.html',
  styleUrls: ['./wheel-history.component.scss']
})

export class WheelHistoryComponent implements OnInit {
...
id:any;
  ...
  constructor(private conn: ConnectionService, private alert: ToastrService, private route: Router, private actRoute: ActivatedRoute) {
      if (actRoute.snapshot.url[1].path =='wheel') {
  ...
  details(){
    var obj={'_id':this.id,game:'wheel'};
    this.conn.postRequest('history/wheelDetails',obj,this.token).subscribe((resData:any) => {
